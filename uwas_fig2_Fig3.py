...
def generate_figure(feats, weights, history, n_total, auc_val,
    ...
    # Value labels — position on the outer end of each bar
    for i, (w, bar) in enumerate(zip(sorted_weights, bars)):
        if w >= 0:
            ax_b.text(w + max_abs_weight * 0.02, i, '{:+.2f}'.format(w),
                      va='center', fontsize=FS - 3, fontweight='bold', color='#444')
        else:
            ax_b.text(w - max_abs_weight * 0.02, i, '{:+.2f}'.format(w),
                      va='center', ha='right', fontsize=FS - 3, fontweight='bold', color='#444')
    ...
    ax_b.set_xlim(-max_abs_weight * 1.3, max_abs_weight * 1.3)
    ax_b.invert_yaxis()
