...
def generate_figure(elbow_features, elbow_info, elimination_histories,
    ...
    # Strip metric suffixes for cleaner display
    all_labels = [l.replace(' (mean)', '').replace(' (med)', '').replace(' (median)', '')
                  for l in all_labels]
    ...
    gs_outer = gridspec.GridSpec(2, 3, height_ratios=[0.6, 1.6],
    ...
    fig.text(0.02, 0.60, 'B', fontsize=20, fontweight='bold', va='top', ha='left')
    ...
    for j, g in enumerate(active_groups):
        ax_b.get_xticklabels()[j].set_color('black')
...
def main():
    ...
    for gname in GROUP_ORDER:
        ...
        # ── Adaptive knowledge: only penalize features whose direction
        #    contradicts knowledge IN THIS SPECIFIC SUBGROUP ──
        ...
        if HAS_BETA_KNOWLEDGE and (main_cache or network_cache):
            # Inject β₃ MRI priors for selected features
            if beta3_regions:
                max_abs_b3 = max(abs(b) for b in beta3_regions.values()) if beta3_regions else 1
                for col in feats:
                    if col in clinical_set or col in main_cache:
                        continue
                    cl = col.lower()
            ...
            # Build priors, then check per-feature consistency in this subgroup
            signs, confidences, bias_strengths, prior_betas, k_report = build_knowledge_priors(
                feats, main_cache=main_cache, network_cache=network_cache, verbose=False)

            # Adaptive: zero out penalty for features already consistent in this group
            n_consistent, n_corrected = 0, 0
            for j, f in enumerate(feats):
                if signs[j] == 0 or confidences[j] < 0.3:
                    continue  # unconstrained features — skip
                # Check raw correlation with AD in this subgroup
                xraw = df_g[f].apply(pd.to_numeric, errors='coerce')
                corr = xraw.corr(y_g)
                if np.isnan(corr):
                    continue
                data_sign = +1 if corr > 0 else -1
                if data_sign == signs[j]:
                    # Data agrees with knowledge → no correction needed
                    bias_strengths[j] = 0.0
                    prior_betas[j] = 0.0
                    n_consistent += 1
                else:
                    # Data contradicts knowledge → apply correction
                    n_corrected += 1

            print(f"    Knowledge: {n_consistent} consistent (no penalty), "
                  f"{n_corrected} corrected (prior-pull applied)")
            for j, f in enumerate(feats):
                if bias_strengths[j] > 1.5:
                    s = {+1:'+', -1:'-', 0:'0'}[int(signs[j])]
                    print(f"      ↑ {annotate_feature(f):30s} sign={s}, "
                          f"conf={confidences[j]:.2f}, μ={prior_betas[j]:+.2f}")
            ...
            clf.fit(Xsc, y_g.values.astype(float))
            weights = dict(zip(feats, np.abs(clf.coef_.ravel())))
            betas = dict(zip(feats, clf.coef_.ravel()))
        else:
            clf = LogisticRegression(C=1.0, penalty='l2', max_iter=2000, random_state=42)
            clf.fit(Xsc, y_g)
    ...
    # Save report
    report = {
        'model_type': 'adaptive_knowledge_constrained' if HAS_BETA_KNOWLEDGE else 'standard_l2',
